    ...
    for i in trange(120, desc='Simulating day'):
        ...
        if i==0:
            ...
            if obs.time_day[...,0]>20:
                action = 1
                # tutorial doesn't allow stricter regulations after 20 days, which doesn't resemble other cities
                if obs.critical_above_threshold:
                    action = 2
            elif not obs.critical_above_threshold:
                action = 0
            else:
                action = 4
            ########################################################################################################################################
